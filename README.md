...
## Problem Statement
네트워크를 통해 전달되는 데이터는 항상 신뢰할 수 없는 환경을 지나갑니다.
서버, 네트워크, 중간자는 언제든지 침해되거나 변조될 수 있습니다.

이 프로젝트는 이러한 환경에서
**서버를 포함한 중간자를 신뢰하지 않아도 되는 통신 구조**를
직접 설계하고 검증해보고자 시작되었습니다.

메시지는 클라이언트에서만 암·복호화되며,
서버는 암호문을 중계하고 저장하는 역할만 수행하도록 설계되었습니다.

## 📌 Overview
이 프로젝트는 TCP 기반 End-to-End Encryption 채팅 프로그램을 구현한 프로젝트입니다. 사용자는 CLI 기반 클라이언트를 통해 상대방과 안전하게 채팅하며, 메시지는 서버를 거치지만 서버는 절대 복호화할 수 없습니다. 모든 암호화/복호화는 클라이언트에서 수행됩니다.
